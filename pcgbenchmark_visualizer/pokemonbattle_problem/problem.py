...
class PokemonBattleProblem(Problem):
    def __init__(self, **kwargs):
        ...
        self._max_turns = kwargs.get("max_turns", 5)
        ...
        self._control_space = DictionarySpace({
            ...
            "rival_pokemon_type": IntegerSpace(self._types_count),
    ...
    def quality(self, info):
        turn_reward = get_range_reward(info["turns"], self._min_turns, self._max_turns)
    ...
    def diversity(self, info1, info2):
        player_diversity = 1 if info1["player_pokemon"] != info2["player_pokemon"] else 0
        rival_diversity = 1 if info1["rival_pokemon"] != info2["rival_pokemon"] else 0
        level_diversity = 1
        ...
        return (player_diversity + rival_diversity + level_diversity) / 3.0
    
    def controlability(self, info, control):
        turn_reward = get_range_reward(info["turns"], control["min_turns"], control["max_turns"])
        ...
        player_effectiveness_reward = get_range_reward(info["player_move_effectiveness"], 0, control["min_player_move_effectiveness"] * 0.5)
        rival_effectiveness_reward = get_range_reward(info["rival_move_effectiveness"], 0, control["min_rival_move_effectiveness"] * 0.5)
